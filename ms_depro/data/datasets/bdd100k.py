...
ALL_CATEGORIES = [
    ...
    ] # traffic light: 0, traffic sign: 1, ..., train: 9



def load_bdd_instances(dirname: str, split: str, depth: str, class_names: Union[List[str], Tuple[str, ...]]):
    """
    Load BDD100K detection annotations to Detectron2 format.
...
def register_bdd_dataset(name, dirname, split, depth, year=2012, class_names=ALL_CATEGORIES):
    DatasetCatalog.register(name, lambda: load_bdd_instances(dirname, split, depth, class_names))
    MetadataCatalog.get(name).set(
        thing_classes=list(class_names), dirname=dirname, year=year, split=split
    )
    
def register_bdd100k(dataset_root, depth):
    SPLITS = [
        ("daytime_train", "BDD100K/Daytime", "train"),
        ("daytime_val", "BDD100K/Daytime", "val"),
        ("night_train", "BDD100K/Night", "train"),
        ("night_val", "BDD100K/Night", "val"),
        ("dawndusk_train", "BDD100K/DawnDusk", "train"),
        ("dawndusk_val", "BDD100K/DawnDusk", "val")
    ]
    for name, dirname, settype in SPLITS:
        register_bdd_dataset(name, osp.join(dataset_root, dirname), settype, depth)
        MetadataCatalog.get(name).evaluator_type = "coco"
        
def register_caronly_bdd100k(dataset_root, depth):
    SPLITS = [
        ("daytime_caronly_train", "BDD100K/Daytime", "train"),
        ("daytime_caronly_val", "BDD100K/Daytime", "val")
    ]
    for name, dirname, settype in SPLITS:
        register_bdd_dataset(name, osp.join(dataset_root, dirname), settype, depth,
                             year=2012, class_names=['car']) # caronly
        MetadataCatalog.get(name).evaluator_type = "coco"
