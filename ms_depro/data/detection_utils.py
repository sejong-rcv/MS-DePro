...
from ms_depro.data.ACVCGenerator import ACVCGenerator
...
def build_strong_augmentation(cfg, is_train):
    """
    This SimCLR-style augmentation used for MSDA
    """
    ...
    if is_train:
        ...
        logger.info("Strong augmentations for training: " + str(augmentation))
    return transforms.Compose(augmentation)


class build_corrupt_augmentation:
    """
    This ImageNet-C augmentation used for MSDG
    """
    def __init__(self, cfg, n_augs=1, is_train=True):
        self.cfg = cfg
        self.n_augs = n_augs
    ...
    def __call__(self, img):
        ...
        crs = random.sample(self.corrupt_func, self.n_augs)
        for c in crs:
            s = random.randint(1, 5)
            image = self.acvc.apply_corruption(image, c, s)
        return image