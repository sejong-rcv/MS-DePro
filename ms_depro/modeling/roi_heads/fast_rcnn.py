...
from ms_depro.layers.soft_nms import batched_soft_nms
from ms_depro.modeling.roi_heads.clip_ms_depro import ReturnLearnablePrompt
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.metanet_shallow_features = metanet_shallow_features
        ...
        if self.use_clip_cls_emb: # use CLIP text embeddings as classifier's weights
            ...
            text_emb_require_grad = False # freeze
            self.use_bias = False
            self.temperature = openset_test[2] # 0.01 is default for CLIP
            
            # class embedding
            if self.learnable_prompt:
                self.return_learnable_prompt = ReturnLearnablePrompt(classnames, 
                                                                     language_encoder, 
                                                                     csc, 
                                                                     agnosticnet, 
                                                                     specificnet, 
    ...
    def from_config(cls, cfg, input_shape):
        if cfg.MODEL.CLIP.LEARNABLE_PROMPT:
            ...
            _weights = {}
            pre_weights = torch.load(cfg.MODEL.BACKBONE_WEIGHTS, map_location="cpu")["model"]
            for k, w in pre_weights.items():
                if k.startswith('lang_encoder'):
                    k = k.replace('lang_encoder.','')
                    _weights[k] = w
            language_encoder.load_state_dict(_weights, strict=False)
            del _weights
        else:
            language_encoder = None
        return {
            ...
            "learnable_prompt": cfg.MODEL.CLIP.LEARNABLE_PROMPT,     
            "csc": cfg.MODEL.CLIP.CSC,       
            "agnosticnet": cfg.MODEL.CLIP.AGNOSTICNET,   
            "specificnet": cfg.MODEL.CLIP.SPECIFICNET,
            "specificnet_from_bbox": cfg.MODEL.CLIP.SPECIFICNET_FROM_BBOX,  
            "specificnet_test_with_buffer": cfg.MODEL.CLIP.SPECIFICNET_TEST_WITH_BUFFER,  
            "metanet_shallow_features": cfg.MODEL.CLIP.METANET_SHALLOW_FEATURES, 
            "learnable_bg": cfg.MODEL.CLIP.LEARNABLE_BG,   
            "language_encoder": language_encoder,
            "classnames": cfg.DATALOADER.CLASS
            # fmt: on
        }

    def forward(self, x, domain_label, is_inference=False, agnostic_feats=None, specific_feats=None):
        """
        Args:
        ...
        if self.use_clip_cls_emb: 
            ...
            # training or closed-set model inference
                ...
                if self.learnable_prompt: 
                    if self.agnosticnet or self.specificnet:
                        agnostic_feats = F.normalize(agnostic_feats, p=2.0, dim=1) if agnostic_feats is not None else None
                        specific_feats = F.normalize(specific_feats, p=2.0, dim=1) if specific_feats is not None else None
                        if self.metanet_shallow_features:
                            self.cls_score = self.return_learnable_prompt(domain_label, is_inference, agnostic_feats, specific_feats)
                        else:
                            self.cls_score = self.return_learnable_prompt(normalized_x)
                    else:
                        self.cls_score = self.return_learnable_prompt()
                    cls_scores = normalized_x @ F.normalize(self.cls_score, p=2.0, dim=1).t() 
                else:
    ...
    def losses(self, predictions, proposals):
        ...
        # parse classification outputs (per region)
        gt_classes = (
